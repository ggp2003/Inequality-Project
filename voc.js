(() => {
  ...
  const LABELLED_COUNTRIES = new Set(["US", "GB", "CA", "DE", "JP", "SE"]);
  const formatDollar = d3.format("$,.0s");
  const formatGini = d3.format(".2f");
  const formatSlope = d3.format("+.2e");
  ...
  Promise.all([
    d3.csv("data/voc_efficiency_points.csv", d3.autoType),
    d3.csv("data/voc_efficiency_regressions.csv", d3.autoType),
  ]).then(([points, regressions]) => {
    vocPoints = points;
  ...
  function configureSlider() {
    const slider = d3.select("#voc-year-slider");
    slider
      .attr("min", vocYears[0])
    ...
    renderYearTicks("#voc-year-ticks");
  }

  function configurePlayback() {
    d3.select("#voc-play-button").on("click", () => {
      if (isPlaying) {
        stopPlayback(false);
        return;
      }
  ...
  function configureMetricToggles() {
    d3.selectAll('input[name="voc-weighting"]').on("change", (event) => {
      currentModelType = event.target.value;
      updateVocYear(currentYear, true);
    });

    d3.selectAll('input[name="voc-x-metric"]').on("change", (event) => {
      currentXMetric = event.target.value;
      redrawVocChart();
    });

    d3.selectAll('input[name="voc-y-metric"]').on("change", (event) => {
  ...
  function configureAverageSlider() {
    const slider = d3.select("#voc-average-year-slider");
    slider
      .attr("min", vocYears[0])
      .attr("max", vocYears.at(-1))
    ...
    renderYearTicks("#voc-average-year-ticks");
  }

  function configureAveragePlayback() {
    d3.select("#voc-average-play-button").on("click", () => {
      if (isAveragePlaying) {
        stopAveragePlayback(false);
        return;
      }
  ...
  function configureAverageMetricToggles() {
    d3.selectAll('input[name="voc-average-weighting"]').on("change", (event) => {
      averageModelType = event.target.value;
      updateAverageYear(averageCurrentYear, true);
    });

    d3.selectAll('input[name="voc-average-x-metric"]').on("change", (event) => {
      averageXMetric = event.target.value;
      redrawAverageChart();
    });

    d3.selectAll('input[name="voc-average-y-metric"]').on("change", (event) => {
  ...
  function updateVocYear(year, animate) {
    ...
    d3.select("#voc-chart-title").text("VoC Efficiency-Equality Space");
    d3.select("#voc-year-slider").property("value", year);
    vocChart.yearLabel.text(year);

    const lines = vocChart.regressionLayer
      .selectAll("line")
      .data(regressionsForYear, (d) => d.voc);
    ...
    const points = vocChart.pointLayer
      .selectAll("circle")
      .data(pointsForYear, (d) => d.country);
    ...
    const labels = vocChart.labelLayer
      .selectAll("text")
      .data(visibleLabels, (d) => d.country);
  ...
  function updateAverageYear(year, animate) {
    ...
    d3.select("#voc-average-year-slider").property("value", year);
    ...
    const currentPoints = averageChart.pointLayer
      .selectAll("circle")
      .data(currentData, (d) => d.voc);
    ...
    const labels = averageChart.labelLayer
      .selectAll("text")
      .data(currentData, (d) => d.voc);
  ...
  function updatePlaybackButton() {
    const button = d3.select("#voc-play-button");
  ...
  function updateAveragePlaybackButton() {
    const button = d3.select("#voc-average-play-button");
  ...
  function labelsForViewport(pointsForYear) {
    if (window.innerWidth <= 720) {
      return pointsForYear.filter((d) => LABELLED_COUNTRIES.has(d.country));
    }
  ...
  function pointTitle(d) {
    ...
    const gdpLabel = currentXMetric === "gdp_pc_ppp" ? "GDP per capita (PPP)" : "GDP per capita (unadjusted)";

    return `${d.label} (${d.country}), ${d.year}\n${gdpLabel}: ${d3.format("$,.0f")(
      d[currentXMetric]
  ...
  function averageTitle(d) {
    ...
    const gdpLabel = averageXMetric === "gdp_pc_ppp" ? "GDP per capita (PPP)" : "GDP per capita (unadjusted)";
